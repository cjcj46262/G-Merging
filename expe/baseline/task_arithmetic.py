...
from emr_merge import emr_merge, apply_vector, weight_average, weight_average_train, ties_merge
...
#model
    ...
    parser.add_argument('--merge', type=str, default="weight_average", help="[weight_average, emr_merge, task_arithmetic, ties_merge]")
...
def test_one_dataset(args):
    ...
    for task_vector in task_vectors:
        task_vector.keep_top_20_percent()
    vector_unified_ties = ties_merge(task_vectors)
    ...
    for key in pretrained_state_dict:
        if key not in task_vector_recon:
            ...
            if args.merge == 'emr_merge':
                task_params[key] = pretrained_state_dict[key] + task_vector_recon[key]
            elif args.merge == 'weights_average':
                task_params[key] = pretrained_state_dict[key] + 0.125 * task_vector_sum.vector[key]
            elif args.merge == 'task_arithmetic':
                task_params[key] = pretrained_state_dict[key] + scaling_coef_ * task_vector_sum.vector[key]
            elif args.merge == 'ties_merge':
                task_params[key] = pretrained_state_dict[key] + scaling_coef_ties * vector_unified_ties[key]
            else:
                print(f'Warning: Unrecognized merge strategy {args.merge} for key {key}')
...
def main(args):
    ...
    for dataset_name, num_tasks in zip(list_datasets, list_num_tasks):
        ...
        all_acc.append(acc)
        with open(f'./results/{args.gnn_type}_{args.pretrain_strategy}/{args.merge}.txt', 'a') as file:
