...
def train(args, model, finetune_model, loader, optimizer, loss_func, finetune_getter, target_getter1, target_getter2,
          backbone_regularization=None):
    # alpha_model.train()
    # finetune_model.eval()
    all_fea_loss = 0.
    all_gtot_loss = 0.
    ...
    for batch in loader:
        ...
        intermediate_output_s, feature_s = finetune_getter(batch.x, batch.edge_index, batch.edge_attr,
        ...
        for i in range(args.num_layer):
            intermediate_output_t[f'gnn.gnns.{i}.mlp.2'] = intermediate_output_t1[f'gnn.gnns.{i}.mlp.2'] - \
...
def eval(args, model, loader):
    ...
    for batch in loader:
        ...
        with torch.no_grad():
            ...
            logits, feature = model(batch.x, batch.edge_index, batch.edge_attr, batch.batch)
    ...
    sample_indices = np.random.choice(features.shape[0], size=1000, replace=True)
    features = features[sample_indices]

    labels = np.full(features.shape[0], args.index)

    avg_roc = cal_roc(y_true, y_scores)

    return avg_roc, features, labels
...
def test_one_dataset_ft(args):
    ...
    model.load_state_dict(dict_para)
    ...
    # # pretrained_path = '/public24_data/cj/CcccJjjj/model_gin/supervised_contextpred.pth'
    # task_vectors = [
    ...
    te_acc, features, labels = eval(args, model, test_loader)
    print(f'test acc:{te_acc:.2f} ')
    # print(te_acc)
    return te_acc, features, labels


def test_one_dataset_pretrain(args):
    set_seed(args.seed)
    model_file = f'/HDDDATA/CcccJjjj/ftmodels/{args.gnn_type}_{args.pretrain_strategy}/{args.gnn_type}_{args.dataset}_sd0.pt'
    print(model_file)
    dict_m = torch.load(model_file, map_location='cpu')
    dict_para = dict_m['model_state_dict']
...
def main(args):
    ...
    for index, (dataset_name, num_tasks) in enumerate(zip(list_datasets, list_num_tasks)):
        ...
        acc, features, labels = test_one_dataset_ft(args)
        all_acc.append(acc)
    ...
    X = np.concatenate(list_features, axis=0)
    y = np.concatenate(list_labels, axis=0)
    ...
    colors = plt.cm.get_cmap('tab10', len(unique_labels))  # 使用 tab10 颜色映射

    for i, label in enumerate(unique_labels):
        idx = (y == label)
        plt.scatter(X_tsne[idx, 0], X_tsne[idx, 1],
                    label=str(label), color=colors(i), s=20)

    # plt.title("Feature Visualization cross Tasks")
    # plt.xlabel("")
    # plt.ylabel("Dim 2")
    # plt.legend(title="Class", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"tsne_1000.pdf", dpi=300, bbox_inches='tight', pad_inches=0)
    plt.close()
