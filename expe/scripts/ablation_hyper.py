...
def train(args, model, finetune_model, loader, optimizer, loss_func, finetune_getter, target_getter1, target_getter2,
    ...
    for batch in loader:
        ...
        if args.gnn_type == 'gcn':
            for i in range(args.num_layer):
                intermediate_output_t[f'gnn.gnns.{i}'] = intermediate_output_t1[f'gnn.gnns.{i}'] - \
                                                         intermediate_output_t2[f'gnn.surgery_mlps.{i}.2']
...
def test_one_dataset(args):
    ...
    task_vectors = [
        TaskVector(pretrained_path,
                   f'/public24_data/cj/CcccJjjj/ftmodels/{args.gnn_type}_{args.pretrain_strategy}/{args.gnn_type}_{dataset_name}_sd0.pt')
...
def train_one_adapters(args):
    ...
    for epoch in range(args.epochs):
        gtot_loss, fea_loss = train(args, model, finetune_model, train_loader, optimizer, loss_func, finetune_getter,
                                    target_getter1, target_getter2, backbone_regularization)
        print(f'epoch {epoch} finished, gtot loss: {gtot_loss}, fea loss: {fea_loss}')
    ...
    torch.save(checkpoint,
               f"./shell1/{args.gnn_type}_{args.pretrain_strategy}/taskArith_surgeryV2_GTOT/{args.dataset}_aligners.pth")
    print(f'test acc:{te_acc:.2f} ')
    # print(te_acc)
    return te_acc
...
def main(args):
    ...
    for index, (dataset_name, num_tasks, batch_size, lr_graph, lr_node) in enumerate(
        ...
        all_acc.append(acc)
        with open(f'./shell1/{args.gnn_type}_{args.pretrain_strategy}/AGMM_{args.alpha:.0e}_{args.lamweight}.txt', 'a') as file:
