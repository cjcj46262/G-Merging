...
## loss balance
    parser.add_argument('--alpha', type=float, default=1, help="balance parameter for clustering")
...
def test_one_dataset(args):
    ...
    for i, d_name in enumerate(list_datasets):
        model_file = f'./ablation/{args.gnn_type}_{args.pretrain_strategy}/AGMM_rank{args.rank}/{d_name}_aligners.pth'
...
def train_one_adapters(args):
    ...
    for param in model.surgery_mlp.parameters():
        param.requires_grad = True
    for param in model.gnn.surgery_mlps.parameters():
        param.requires_grad = True
    ...
    os.makedirs(f'./ablation/{args.gnn_type}_{args.pretrain_strategy}/AGMM_rank{args.rank}', exist_ok=True)
    torch.save(checkpoint, f"./ablation/{args.gnn_type}_{args.pretrain_strategy}/AGMM_rank{args.rank}/{args.dataset}_aligners.pth")
    print(f'test acc:{te_acc:.2f} ')
    # print(te_acc)
    return te_acc
...
def main(args):
    ...
    for index, (dataset_name, num_tasks, batch_size, alpha, lr_graph, lr_node) in enumerate(zip(list_datasets, list_num_tasks, list_batch_sizes, list_alpha, list_lr_graph, list_lr_node)):
        ...
        all_acc.append(acc)
        with open(f'./ablation/{args.gnn_type}_{args.pretrain_strategy}/testtime1.txt', 'a') as file:
